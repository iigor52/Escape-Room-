...
STATION_DATA = {
    "station_number": 2,
    "station_name": "ICU Escape Room - Station 2",
    "groups": {
        "group1": {
            "name": "Group 1",
            "next_station": 3,
    ...
    if not st.session_state.quiz_completed:
        ...
        with help_col1:
            ...
            if st.button(hint_button_text, use_container_width=True, key="hint_btn"):
                ...
                if not wrong_positions:
                    hint_text = "Enter code to analyze or all answers are already correct"
        ...
        with help_col2:
            ...
            if st.button(answer_button_text, use_container_width=True, key="answer_btn"):
                ...
                if next_wrong_idx is None:
                    ans_text = "All incorrect answers have been revealed or code is correct"
                else:
                    # Add this position to revealed list
                    if next_wrong_idx not in st.session_state.revealed_answers:
                        st.session_state.revealed_answers.append(next_wrong_idx)
        ...
        # BANNERS DIRECTLY BELOW HELP BUTTONS - most important change!
        now = datetime.now()

        # Hint banner - improved cache control
        if (st.session_state.hint_until and 
            ...
            st.markdown('</div>', unsafe_allow_html=True)
        elif st.session_state.hint_until and now >= st.session_state.hint_until:
            # Clean up expired hint
            st.session_state.hint_until = None
            st.session_state.hint_text = ""

        # Answer banner - improved cache control
        if (st.session_state.answer_until and 
            ...
            st.markdown('</div>', unsafe_allow_html=True)
        elif st.session_state.answer_until and now >= st.session_state.answer_until:
            # Clean up expired answer
            st.session_state.answer_until = None
            st.session_state.answer_text = ""
